...
function runSession(
  ...
  options: Partial<ConstructorParameters<typeof OrpSession>[2]> = {}
) {
  const snapshotTailCountThreshold = options.snapshotTailCountThreshold ?? 6;
  ...
  const session = new OrpSession(initiator, responder, {
    ...
    chunkTransferThreshold: options.chunkTransferThreshold ?? 4,
    snapshotTailCountThreshold,
    roundLimit: options.roundLimit ?? 64,
  });
  return { initiator, responder, session, result: session.run() };
}

describe("OrpSession", () => {
  it("repairs a small diff through operation exchange", () => {
    const { initiator, responder, session, result } = runSession(
      [{ docHandle: "doc-small", values: ["alpha", "beta"] }],
      [{ docHandle: "doc-small", values: ["alpha"] }],
      { chunkTransferThreshold: 8, snapshotTailCountThreshold: 6 }
    );

    expect(result.differingDocs).toEqual([
      expect.objectContaining({ docHandle: "doc-small" }),
    ]);
    expect(result.documents).toEqual([
      expect.objectContaining({
        ...
        responderMissingOpIds: expect.any(Array),
      }),
    ]);
    expect(result.transcript.map((entry) => entry.message.type)).toEqual(
      expect.arrayContaining(["orp/hello", "orp/inventory-done", "orp/doc-frame", "orp/blob-get", "orp/blob-put"])
    );
    expect(session.getState()).toEqual({
      phase: "complete",
      currentDoc: undefined,
      completedDocs: ["doc-small"],
      transcriptLength: result.transcript.length,
    });
    expect(initiator.materialize()).toEqual(responder.materialize());
  });

  it("uses chunk transfer when many operations are localized to a few chunks", () => {
    const { initiator, responder, result } = runSession(
      [{ docHandle: "doc-bulk", values: ["a-1", "a-2", "a-3", "a-4", "a-5", "a-6"] }],
      [{ docHandle: "doc-bulk", values: ["a-1"] }],
      { chunkTransferThreshold: 2, snapshotTailCountThreshold: 10 }
    );
  ...
  it("uses snapshot transfer before op repair for large divergence", () => {
    const { initiator, responder, result } = runSession(
      [{ docHandle: "doc-snapshot", values: ["v-1"] }],
      [{ docHandle: "doc-snapshot", values: ["v-1", "v-2", "v-3", "v-4", "v-5", "v-6", "v-7"] }],
      { chunkTransferThreshold: 99, snapshotTailCountThreshold: 4 }
    );

    expect(result.documents).toEqual([
      expect.objectContaining({
        docHandle: "doc-snapshot",
        strategy: "snapshot",
        snapshotProvider: "responder",
    ...
    expect(result.transcript.map((entry) => entry.message.type)).not.toContain("orp/doc-frame");
    expect(initiator.materialize()).toEqual(responder.materialize());
  });
});
